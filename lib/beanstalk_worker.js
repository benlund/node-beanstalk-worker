...
BeanstalkWorker.prototype.handle_next = function() {
  ...
  this.connection.reserve_with_timeout(5).addCallback(function(job_id, job_json) {
	...
	(function() {
	try {
	  var job_timedout = false;
	  handler(job.data, function() {
	    duration = new Date().getTime() - start;
	    self.log('Ran job : ' + job_id + ' in ' + duration + 'ms (' + job_json + ')');
	    if(job_timedout) {
	      // incase it ever callsback, do nothing, already emitted next  -- @@fixme
	    }
	    else {
	  ...
	  function(reason) {
	    if(reason === 'timedout') { //@@ fix this
	      job_timedout = true;
	      self.log('Timed out job : ' + job_id);
	      self.release_and_emit_next(job_id);
	    }
	    else {
	      self.log('Failed to run job : ' + job_id + ' : ' + reason);
...
BeanstalkWorker.prototype.release_and_emit_next = function(job_id) {
  var self = this;
  self.connection.release(job_id, bt.Client.LOWEST_PRIORITY, 30).addCallback(function() { //@@ hardcoded delay time @@ fixme
    self.log('Released ' + job_id);
    self.emit('next');
  }).addErrback(function(reason) {
    self.handle_error('releasing ', [job_id, reason]);
    self.emit('next');
  });
};
